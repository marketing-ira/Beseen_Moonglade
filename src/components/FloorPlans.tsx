...
function FloorPlans({ setIsModalShow  ,setIsModalTitle}: FloorPlansPropsType) {
  ...
  // Lazy load staggered animation for floor plan cards
  const [containerRef, visibleIndices] = useLazyLoadStaggered(plansOrder.length, 100);
          ...
          {plansOrder.map((p, index) => {
            ...
            const content = (
              <div
                ...
                className={`relative w-full aspect-[7/6] overflow-hidden rounded-xl cursor-pointer transition-all duration-500 ${
